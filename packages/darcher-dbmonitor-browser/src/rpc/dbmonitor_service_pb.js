 ...
 * @fileoverview
 * @enhanceable
 * @suppress {missingRequire} reports error on implicit type usages.隐式类型使用的错误
 * @suppress {messageConventions} JS Compiler reports an error if a variable or
 *     field starts with 'MSG_' and isn't a translatable message. 如果一个变量或字段以'MSG_'开头并且不是一个可翻译的消息，JS Compiler会报告一个错误。
 * @public
 */
 ...
 * valid.
 * 可选的初始数据数组，通常来自服务器响应，或直接用Javascript构造。
该数组在适当的位置使用，并成为构造的对象的一部分。
它不是克隆的。
如果没有提供数据，则构造的对象将为空，但仍然有效。
 ...
 * given reader into the given message object. 从reader中反序列化成message对象
...
if (jspb.Message.GENERATE_TO_OBJECT) {
/**
 * Creates an object representation of this proto.创建该协议的对象形式
 * Field names that are reserved in JavaScript and will be renamed to pb_name.在JavaScript中保留的字段名将被重命名为pb_xx
 * Optional fields that are not set will be set to undefined.没有设置的字段被设置为未定义
 * To access a reserved field use, foo.pb_<name>, eg, foo.pb_default.访问保留字段的使用
 * For the list of reserved names please see:
 *     net/proto2/compiler/js/internal/generator.cc#kKeyword.
 * @param {boolean=} opt_includeInstance Deprecated. whether to include the
 *     JSPB instance for transitional soy proto support:
 ...
 * @param {!proto.darcher.TableContent} msg The message object to deserialize into.反序列化生成的对象
 * @param {!jspb.BinaryReader} reader The BinaryReader to use.使用的二进制读取器
 * @return {!proto.darcher.TableContent}
 */
 ...
 * Optional fields that are not set will be set to undefined.
 * To access a reserved field use, foo.pb_<name>, eg, foo.pb_default.
 * For the list of reserved names please see:保留字段名的列表查看
 *     net/proto2/compiler/js/internal/generator.cc#kKeyword.
 * @param {boolean=} opt_includeInstance Deprecated. whether to include the
 *     JSPB instance for transitional soy proto support:
 *     http://goto/soy-param-migration
